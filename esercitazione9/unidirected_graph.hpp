...
public:
	...
	/* Costruttore in cui non viene specificato il peso in modo da mantenere il costruttore dell'ultima esercitazione */
	unidirected_edge(const T& nodo1, const T& nodo2)
	: unidirected_edge(nodo1, nodo2, 1.0)
	{}
	
	/* Costruttore user-defined col peso */
	unidirected_edge(const T& nodo1, const T& nodo2, double weight)
		:peso(weight)
	{
		if (nodo1 <= nodo2) {
			nodo_from = nodo1;
